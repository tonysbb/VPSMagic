    ...
    if command -v docker >/dev/null 2>&1; then
      while IFS= read -r cid; do
        ...
        if ! log_dry_run "docker exec ${cname} mysqldump --all-databases"; then
          # 尝试无密码或从环境变量获取密码
          local mysql_pass
          mysql_pass="$(docker inspect "${cid}" --format '{{ range .Config.Env }}{{ . }} {{ end }}' 2>/dev/null | grep -oP 'MYSQL_ROOT_PASSWORD=\K[^ ]+' || echo "")"
      ...
      done < <(docker ps -q --filter "ancestor=mysql" --filter "ancestor=mariadb" 2>/dev/null; \
               docker ps -q --filter "name=mysql" --filter "name=mariadb" 2>/dev/null)
    fi
    ...
    if command -v mysqldump >/dev/null 2>&1 && [[ -n "${DB_MYSQL_HOST_USER:-}" ]]; then
      ...
      if ! log_dry_run "mysqldump --all-databases"; then
        mysqldump --all-databases --single-transaction \
          ...
          2>/dev/null > "${mysql_dir}/host_all.sql" || {
  ...
  _collect_postgres() {
    ...
    if command -v docker >/dev/null 2>&1; then
      while IFS= read -r cid; do
        ...
        if ! log_dry_run "docker exec ${cname} pg_dumpall"; then
          docker exec "${cname}" pg_dumpall -U postgres 2>/dev/null \
            > "${pg_dir}/${cname}_all.sql" || true
          ((dumped++))
        fi
      done < <(docker ps -q --filter "ancestor=postgres" 2>/dev/null; \
    ...
    if command -v pg_dumpall >/dev/null 2>&1; then
      ...
      if ! log_dry_run "pg_dumpall"; then
        sudo -u postgres pg_dumpall 2>/dev/null > "${pg_dir}/host_all.sql" || {
